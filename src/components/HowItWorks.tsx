...
import { CheckCircle2, SmartphoneNfc, ShoppingCart, CreditCard, Package, Truck, MessageSquare } from 'lucide-react';
...
const Step = ({ icon, title, description, index, isActive }: StepProps) => {
  return (
    <div className={`relative flex items-start gap-4 pb-8 ${index === 5 ? 'border-0' : 'border-l'} border-border/50 ml-3`}>
      <div 
        className={`absolute top-0 left-0 w-6 h-6 rounded-full -ml-3 z-10 flex items-center justify-center transition-all duration-300 ${
          isActive ? 'bg-orderbot text-white scale-110' : 'bg-muted text-foreground/30'
        }`}
      >
        {isActive ? <CheckCircle2 size={16} /> : <span className="text-xs font-semibold">{index + 1}</span>}
      </div>
      
      <div className={`pt-0 pl-6 transition-all duration-300 ${isActive ? 'opacity-100' : 'opacity-60'}`}>
        <div className="flex items-center gap-3 mb-2">
          <div className={`p-2 rounded-lg ${isActive ? 'bg-orderbot/10 text-orderbot' : 'bg-muted text-foreground/50'}`}>
            {icon}
          </div>
  ...
  const steps = [
    {
      ...
      description: "Customer initiates a conversation with the business's WhatsApp bot."
    },
    {
      icon: <ShoppingCart size={20} />,
      title: "Browse Products & Select Items",
      description: "Bot provides a menu/catalog, and customer adds items to their cart."
    },
    {
    ...
    },
    {
      ...
      description: "Bot verifies payment and notifies the business of the new order."
    },
    {
      icon: <Package size={20} />,
      title: "Order Processing",
      description: "Business processes the order and updates status in the admin panel."
    },
    {
      icon: <Truck size={20} />,
      title: "Delivery & Completion",
      description: "Customer receives updates and order is marked as completed upon delivery."
    }
  ];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          <div>
            ...
            >
              ...
              >
                From initial chat to delivery confirmation, Orderbot provides a seamless end-to-end experience for your customers.
              </p>
          ...
          >
            ...
            <div className="absolute -bottom-6 -left-6 glass rounded-xl p-4 shadow-soft-xl max-w-[240px]">
              ...
              <p className="text-sm text-foreground/70">Your order #1234 has been confirmed. It will be ready in 20 minutes.</p>
            </div>
            ...
            <div className="absolute -top-6 -right-6 glass rounded-xl p-4 shadow-soft-xl max-w-[240px]">
              ...
              <p className="text-sm text-foreground/70">Payment of $35.99 has been successfully processed.</p>
            </div>
          </div>
        </div>
      </div>
